...
use App\SapiSubResult;
use App\SapiSubFuzzyResult;
use Charts;
use Flash;
...
class ResultController extends Controller
{
    ...
    public function index(){
        ...
        $pPluck = Sapi::pluck('nama_sapi', 'id');
        $expert = Expert::all();
        ...
        $sapiSubResult = SapiSubResult::all();
        $sapiSubFuzzyResult = SapiSubFuzzyResult::all();
        $sapis = Sapi::all();
        $exp = new Expert();
        ...
        return view('result.kriteria')->with('data', $expertKriteriaComparison)
                ...
                ->with('hasilSapi', $sapiSubResult)
                ->with('hasilFuzzySapi', $sapiSubFuzzyResult)
    ...
    public function storeSapi(){
      $hasil = false;
      $ka = [];
      SapiSubResult::truncate();
      $kriterias = Kriteria::all();
      $sapis = Sapi::all();
      $experts    = Expert::all();
      $sapiSubs = SapiSub::orderBy('expert_id','ASC')->orderBy('kriteria_id','ASC')->orderBy('sub_kriteria_id','ASC')->orderBy('sapi1_id','ASC')->get();
      $subKriterias = SubKriteria::all();

      $dataKriteria = null;

      foreach ($experts as $e) {
        $kk = $sapiSubs->where('expert_id', '=', $e->id);
          ...
          foreach ($sss as $subkey => $subvalue) {
            $kks = $kkr->where('sub_kriteria_id','=', $subvalue->id);
              ...
              foreach ($sapis as $k1) {
                ...
                foreach ($sapis as $k2) {
                  $found1 = $kks->where('sapi1_id', '=', $k1->id)->where('sapi2_id', '=', $k2->id)->first();
                  $found2 = $kks->where('sapi1_id', '=', $k2->id)->where('sapi2_id', '=', $k1->id)->first();
    ...
    public function storeFuzSapi(){
      $hasil = false;
      SapiSubFuzzyResult::truncate();
      $kriterias = Kriteria::all();
      $sapis = Sapi::all();
      $experts    = Expert::all();
      $sapiSubs = SapiSub::orderBy('expert_id','ASC')->orderBy('kriteria_id','ASC')->orderBy('sub_kriteria_id','ASC')->orderBy('sapi1_id','ASC')->get();
